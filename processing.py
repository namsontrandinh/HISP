...
if __name__ == '__main__':
    ...
    if len(dataset_name) != 3:
        ...
        sys.exit(1)
    # DA SUA: cho phep them ten dataset tong hop (synth-*) ngoai 3 ten
    # goc, de dung duoc voi 130 do thi tu sinh (theo chi dao train tren
    # bo data tu sinh, khong can Extended/Celebrity/WannaCry rieng).
    if dataset_name[0] not in ['Extended', 'Celebrity', 'WannaCry'] \
            and not dataset_name[0].startswith('synth-'):
    ...
    if dataset_name[1] == 'train':
        ...
        for var in ["X", "y", "sX", "sy"]:
            with gzip.open('datadir/{}_train_{}_{}_random.pkl.gz'.format(dataset_name[0], dataset_name[2], var),
                           'wb') as f_out:
                pickle.dump(getattr(data, var), f_out, protocol=4)

        data.makedata(sys.argv[1].strip(), 1601, 400)
        for var in ["sX", "sy"]:
            with gzip.open('datadir/{}_val_{}_{}_random.pkl.gz'.format(dataset_name[0], dataset_name[2], var),
                           'wb') as f_out:
                pickle.dump(getattr(data, var), f_out, protocol=4)

        data.makedata(sys.argv[1].strip(), 2001, 1600)
        for var in ["X", "y", "sX", "sy"]:
            with gzip.open('datadir/{}_train_{}_{}_degree.pkl.gz'.format(dataset_name[0], dataset_name[2], var),
                           'wb') as f_out:
                pickle.dump(getattr(data, var), f_out, protocol=4)

        data.makedata(sys.argv[1].strip(), 3601, 400)
        for var in ["sX", "sy"]:
            with gzip.open('datadir/{}_val_{}_{}_degree.pkl.gz'.format(dataset_name[0], dataset_name[2], var),
                           'wb') as f_out:
                pickle.dump(getattr(data, var), f_out, protocol=4)
    elif dataset_name[1] == 'test':
        data.makedata(sys.argv[1].strip(), 1, 2000)
        for var in ["X", "y", "sX", "sy"]:
            with gzip.open('datadir/{}_test_{}_{}_random.pkl.gz'.format(dataset_name[0], dataset_name[2], var),
                           'wb') as f_out:
                pickle.dump(getattr(data, var), f_out, protocol=4)

        data.makedata(sys.argv[1].strip(), 2001, 2000)
        for var in ["X", "y", "sX", "sy"]:
            with gzip.open('datadir/{}_test_{}_{}_degree.pkl.gz'.format(dataset_name[0], dataset_name[2], var),
                           'wb') as f_out:
                pickle.dump(getattr(data, var), f_out, protocol=4)
...
def evaluation():
    ...
    with torch.no_grad():
        for tl in test_labels:
            ...
            print('for target graph {}_{} | Accuracy: {}, MAPE: {}'.format(tl[0], tl[2], (len(pairs) - cnt) / len(pairs), mape_sum / (cnt + 1e-9)))
            
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--target", help="gpu number")
    parser.add_argument("--prob", help="gpu number")
    ...
    # Submodularity: Is MONSTOR submoudular as the ground-truth influence function is?

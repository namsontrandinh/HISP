...
int conv_iter[BATCH_SIZE];   // TOI UU: vong ma moi mo phong hoi tu
...
int do_task(int i, int real_seed_size){
	...
	conv_iter[i] = iter-1;   // TOI UU: ghi lai vong hoi tu de bo bot vong tich luy
...
int main(int argc, char **argv){
	omp_set_num_threads(omp_get_max_threads()); // TOI UU: dung het core may co
	...
	while(turn < 4000){
		...
		*/
        ...
        for(int tc=0;tc<TEST_COUNT;tc+=BATCH_SIZE){
			...
			// TOI UU: lan truyen thuong hoi tu sau ~10 vong nhung code cu chay
			// du limit_iter vong -> cac vong sau chi lap lai y het gia tri tai
			// vong hoi tu. Chi tinh that toi maxconv roi cong thang gia tri do
			// cho phan con lai -> ket qua Y HET, bo ~90% cong viec.
			// Dong thoi gop thanh 1 vung song song/batch thay vi hang nghin
			// vung nho (chi phi khoi tao luong ap dao cong viec that).
			int maxconv = 0;
			for(int i=0;i<bsize;i++) if(conv_iter[i] > maxconv) maxconv = conv_iter[i];
			if(maxconv > limit_iter) maxconv = limit_iter;
			if(maxconv < 0) maxconv = 0;

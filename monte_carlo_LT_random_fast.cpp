...
// TOI UU: act chi can iteration HIEN TAI (khong vong nao doc lai act cu),
// nen dung 1 mang/luong thay vi MAX_ITER+1 mang -> bo hang phep copy
// vector<double> 43KB moi vong lap (nguon ton kem lon nhat).
std::vector<double> act[BATCH_SIZE];
int conv_iter[BATCH_SIZE];   // vong ma mo phong i hoi tu
int batch_last[MAX_N];       // dong gop cua batch tai vong hoi tu
...
int do_task(int i, int real_seed_size){
	...
	for(int j=0;j<std::min(n, MAX_N);j++){
		...
		act[i][j] = (double)0.0;   // reset 1 lan/mo phong thay vi copy moi vong
	}
	...
	while(!que[i].empty() && iter <= limit_iter){
		...
		while(lft--){
			...
			for(auto &nxt: E[i][now]){ 
				...
				act[i][nxt.first] += nxt.second;
				//LT cascading behavior
				if(act[i][nxt.first] > threshold[i][nxt.first]){
...
int main(int argc, char **argv){
	omp_set_num_threads(omp_get_max_threads()); // dùng hết core máy có, thay vì hardcode 4
    ...
    while(turn < 2000){
		...
		for(int tc=0;tc<TEST_COUNT;tc+=BATCH_SIZE){
			...
			// TỐI ƯU: gộp thành 1 vùng song song duy nhất mỗi batch,
			// thay vì bsize*(limit_iter+1) vùng nhỏ (bản gốc tạo ~2020
			// vùng song song/batch -> ~10 triệu vùng/turn, chi phí
			// khởi tạo luồng OpenMP áp đảo công việc thực tế bên trong).
			// Kết quả cộng dồn vào tot[][] giống hệt bản gốc, chỉ đổi
			// cách tổ chức vòng lặp.
			// TOI UU: lan truyen thuong hoi tu sau ~10 vong, nhung vong cu
			// chay du 101 vong -> 90 vong sau chi lap lai y het gia tri tai
			// vong hoi tu. Chi tinh that toi maxconv, phan con lai cong
			// thang gia tri do -> ket qua y het, bo ~90% cong viec.
			int maxconv = 0;
			for(int i=0;i<bsize;i++) if(conv_iter[i] > maxconv) maxconv = conv_iter[i];
			if(maxconv > limit_iter) maxconv = limit_iter;

			#pragma omp parallel for collapse(2)
			for(int ii=0;ii<maxconv;ii++){
			...
			for(int jj=0;jj<n;jj++){
				int s = 0;
				for(int i=0;i<bsize;i++){
					s += pi[i][maxconv].test(jj);
				}
				for(int ii=maxconv;ii<=limit_iter;ii++){
					tot[ii][jj] += s;
				}
			}
		}
